...
function GroupsView({ groups, activeGroup, setActiveGroup, onCreate, onJoin, notify, refresh, user, onOpenBills }) {
  ...
  return <div className="group-manager">
    ...
    <section className="group-workspace">{!detail ? <div className="detail-loading"><LoaderCircle className="spin"/>加载群组资料</div> : <>
      ...
      <div className="group-tabs"><button className={view === 'overview' ? 'active' : ''} onClick={() => setView('overview')}><Grid2X2 size={15}/>概览</button><button className={view === 'chat' ? 'active' : ''} onClick={() => setView('chat')}><MessageCircle size={15}/>聊天</button><button className={view === 'activities' ? 'active' : ''} onClick={() => setView('activities')}><CalendarDays size={15}/>活动</button><button className={view === 'collections' ? 'active' : ''} onClick={() => setView('collections')}><CircleDollarSign size={15}/>收款计划</button><button className={view === 'travel' ? 'active' : ''} onClick={() => setView('travel')}><Route size={15}/>旅行路线</button><button className={view === 'members' ? 'active' : ''} onClick={() => setView('members')}><Users size={15}/>成员</button><button className={view === 'invite' ? 'active' : ''} onClick={() => setView('invite')}><UserPlus size={15}/>邀请与审核{detail.join_requests.length > 0 && <b>{detail.join_requests.length}</b>}</button>{canManage && <button className={view === 'settings' ? 'active' : ''} onClick={() => setView('settings')}><Settings size={15}/>设置</button>}<button className={view === 'logs' ? 'active' : ''} onClick={() => setView('logs')}><ScrollText size={15}/>日志</button></div>
      ...
      {view === 'collections' && <CollectionPlansPanel groupId={detail.id} detail={detail} user={user} notify={notify}/>}
...
const collectionStatusLabels = { collecting: '成员录入中', settled: '转账确认中', completed: '全部完成', archived: '已归档' }
const collectionEntryStatusLabels = { pending: '待群主审核', approved: '审核通过', rejected: '未纳入结算' }
const collectionSplitLabels = { host: '自己请客', all: '全员 AA', subset: '部分成员 AA', ratio: '按比例分摊' }

function CollectionPlansPanel({ groupId, detail, user, notify }) {
  ...
  const [activityId, setActivityId] = useState('custom')
  const [participantIds, setParticipantIds] = useState([])
  const [ratios, setRatios] = useState({})
  const isOwner = detail.current_role === 'owner'
  const selectedPlan = plans.find(plan => plan.id === selectedPlanId) || plans[0] || null

  const load = useCallback(async () => {
    try {
      const [nextPlans, nextActivities] = await Promise.all([
        api(`/groups/${groupId}/collection-plans`),
        api(`/groups/${groupId}/activities`),
      ])
      setPlans(nextPlans)
      setActivities(nextActivities)
      setSelectedPlanId(current => nextPlans.some(plan => plan.id === current) ? current : nextPlans[0]?.id || null)
    } catch (error) { notify(error.message) }
  }, [groupId, notify])
  ...
  const perform = async (work, success) => {
    if (busy) return
    setBusy(true)
    try { await work(); await load(); notify(success) }
    catch (error) { notify(error.message) }
    finally { setBusy(false) }
  }
  ...
  const createPlan = event => {
    ...
    perform(
      async () => {
        const created = await api(`/groups/${groupId}/collection-plans`, {method:'POST', body:JSON.stringify({title:data.get('title'), description:data.get('description') || ''})})
        setSelectedPlanId(created.id)
        setShowCreate(false)
        form.reset()
      },
      '收款计划已发起，群成员现在可以提交垫付款',
    )
  }
  ...
  const submitEntry = event => {
    ...
    if (!Number.isFinite(amount) || paidCents <= 0) return notify('请输入正确的付款金额')
    const payload = {
      activity_id: activityId === 'custom' ? null : Number(activityId),
      activity_name: activityId === 'custom' ? String(data.get('activity_name') || '') : '',
      title: data.get('title'), paid_cents: paidCents, split_mode: splitMode,
      participant_ids: splitMode === 'subset' ? participantIds : [],
      ratios: splitMode === 'ratio' ? ratios : {},
    }
    perform(
      async () => {
        await api(`/collection-plans/${selectedPlan.id}/entries`, {method:'POST', body:JSON.stringify(payload)})
        form.reset()
        setSplitMode('all'); setParticipantIds([]); setRatios({}); setActivityId('custom')
      },
      '垫付款已提交，等待群主审核',
    )
  }

  const review = (entryId, decision) => perform(
    () => api(`/collection-entries/${entryId}/review`, {method:'POST', body:JSON.stringify({decision, note:''})}),
    decision === 'approved' ? '已审核通过并纳入待结算金额' : '该记录已拒绝',
  )
  const withdraw = entryId => perform(() => api(`/collection-entries/${entryId}`, {method:'DELETE'}), '已撤回待审核记录')
  const settle = () => {
    if (!selectedPlan) return
    if (selectedPlan.totals.approved_count === 0) {
      notify(selectedPlan.totals.pending_count > 0 ? '请先在下方通过至少一笔待审核垫付款，再进行结算' : '当前没有可结算的已审核款项')
      document.querySelector('.collection-entries')?.scrollIntoView({behavior:'smooth', block:'start'})
      return
    }
    if (!window.confirm('将按当前已审核记录生成转账方案；未审核记录不会纳入本次结算。是否继续？')) return
    perform(() => api(`/collection-plans/${selectedPlan.id}/settle`, {method:'POST'}), '结算完成，转账方案已同步给群成员')
  }
  const confirmTransfer = (transferId, type) => perform(
    () => api(`/collection-transfers/${transferId}/${type}`, {method:'POST'}),
    type === 'confirm-payment' ? '已确认付款，等待收款人确认' : '已确认收款',
  )
  const archive = () => perform(() => api(`/collection-plans/${selectedPlan.id}/archive`, {method:'POST'}), '本次收款计划已归档')
  const toggleParticipant = memberId => setParticipantIds(current => current.includes(memberId) ? current.filter(id => id !== memberId) : [...current, memberId])

  return <section className="collection-page">
    <div className="collection-header"><div><span className="label-chip"><CircleDollarSign size={13}/> 群组收款计划</span><h2>成员分别记账，后台统一结算</h2><p>每笔垫付款先由群主审核，再按整组净额生成最少转账次数的现金流方案；AI 分账仍作为独立功能运行。</p></div>{isOwner && <button className="primary-btn small" disabled={Boolean(plans.find(plan => plan.status === 'collecting'))} onClick={() => setShowCreate(true)}><Plus size={15}/>发起收款计划</button>}</div>
    {!plans.length ? <div className="travel-empty"><CircleDollarSign size={28}/><h3>还没有收款计划</h3><p>{isOwner ? '点击“发起收款计划”，成员就能开始录入各自垫付款。' : '等待群主发起第一笔收款计划。'}</p></div> : <div className="collection-layout">
      <aside className="manager-panel collection-plan-list"><div className="manager-head"><div><h3>全部计划</h3><p>每次结算单独保留</p></div><span>{plans.length} 次</span></div>{plans.map(plan => <button key={plan.id} className={selectedPlan?.id === plan.id ? 'active' : ''} onClick={() => setSelectedPlanId(plan.id)}><div><strong>{plan.title}</strong><small>{formatDate(plan.created_at)} · {plan.totals.approved_count} 笔已审核</small></div><b>{money(plan.totals.approved_cents)}</b><span className={`collection-status ${plan.status}`}>{collectionStatusLabels[plan.status]}</span></button>)}</aside>
      {selectedPlan && <div className="collection-main">
        <div className="manager-panel collection-summary"><div className="manager-head"><div><h3>{selectedPlan.title}</h3><p>{selectedPlan.description || '本计划未填写说明'}</p></div><span className={`collection-status ${selectedPlan.status}`}>{collectionStatusLabels[selectedPlan.status]}</span></div><div className="collection-kpis"><div><span>成员提交</span><strong>{money(selectedPlan.totals.submitted_cents)}</strong><small>{selectedPlan.entries.length} 笔记录</small></div><div><span>审核通过</span><strong>{money(selectedPlan.totals.approved_cents)}</strong><small>{selectedPlan.totals.approved_count} 笔纳入结算</small></div><div><span>待审核</span><strong>{selectedPlan.totals.pending_count}</strong><small>结算时自动排除</small></div><div><span>转账进度</span><strong>{selectedPlan.totals.completed_transfers}/{selectedPlan.totals.transfer_count}</strong><small>需付款与收款双确认</small></div></div>{isOwner && selectedPlan.status === 'collecting' && <><button className="primary-btn small" disabled={busy} onClick={settle}><ClipboardCheck size={15}/>{selectedPlan.totals.approved_count ? '立即结算已审核款项' : '审核后立即结算'}</button>{selectedPlan.totals.approved_count === 0 && <p className="collection-settle-hint">{selectedPlan.totals.pending_count > 0 ? '还有待审核记录，请先在下方点击“通过”；审核通过后即可结算。' : '当前没有可结算的已审核款项。'}</p>}</>}{isOwner && selectedPlan.status === 'completed' && <button className="secondary-btn" disabled={busy} onClick={archive}><CheckCircle2 size={15}/>归档本次计划</button>}</div>

        {selectedPlan.status === 'collecting' && <form className="manager-panel collection-entry-form" onSubmit={submitEntry}><div className="manager-head"><div><h3>提交我的垫付款</h3><p>付款人固定为当前登录账号，提交后等待群主审核</p></div><CircleDollarSign size={18}/></div><div className="collection-form-grid"><label>所属活动<select value={activityId} onChange={event => setActivityId(event.target.value)}><option value="custom">其他活动 / 手动填写</option>{activities.map(activity => <option key={activity.id} value={activity.id}>{activity.title}</option>)}</select></label>{activityId === 'custom' && <label>活动名称<input name="activity_name" required maxLength="120" placeholder="例如：周末露营"/></label>}<label>款项说明<input name="title" required maxLength="120" placeholder="例如：营地预付款"/></label><label>我实际支付<input name="amount" type="number" min="0.01" max="10000000" step="0.01" required placeholder="0.00"/></label></div><div className="collection-split"><span>这笔账如何承担</span><div className="collection-segments">{Object.entries(collectionSplitLabels).map(([key, label]) => <button type="button" key={key} className={splitMode === key ? 'active' : ''} onClick={() => setSplitMode(key)}>{label}</button>)}</div></div>{splitMode === 'host' && <p className="collection-mode-hint">这笔费用全部由你承担，不会让其他成员向你付款。</p>}{splitMode === 'all' && <p className="collection-mode-hint">提交时的全部 {selectedPlan.members.length} 位群成员平均承担，无法整除的分会按固定顺序分配。</p>}{splitMode === 'subset' && <div className="collection-member-options">{selectedPlan.members.map(member => <label key={member.id}><input type="checkbox" checked={participantIds.includes(member.id)} onChange={() => toggleParticipant(member.id)}/><Avatar initials={member.initials} color={member.avatar_color}/><span>{member.display_name}</span></label>)}</div>}{splitMode === 'ratio' && <div className="collection-ratio-grid">{selectedPlan.members.map(member => <label key={member.id}><span>{member.display_name}</span><input type="number" min="0" step="0.01" value={ratios[member.id] || ''} onChange={event => setRatios(current => ({...current, [member.id]: event.target.value}))} placeholder="比例"/></label>)}</div>}<button className="primary-btn small" disabled={busy}>{busy ? '提交中...' : '提交给群主审核'} <ArrowUpRight size={15}/></button></form>}

        <div className="manager-panel collection-entries"><div className="manager-head"><div><h3>成员垫付款</h3><p>金额与承担人完整公开，审核状态实时同步</p></div><span>{selectedPlan.entries.length} 笔</span></div>{selectedPlan.entries.map(entry => <article key={entry.id}><div className="collection-entry-head"><div><strong>{entry.title}</strong><small>{entry.payer_name} · {entry.activity_name} · {formatDate(entry.created_at)}</small></div><b>{money(entry.paid_cents)}</b><span className={`collection-entry-status ${entry.status}`}>{collectionEntryStatusLabels[entry.status]}</span></div><div className="collection-entry-detail"><span>{collectionSplitLabels[entry.split_mode]}</span><p>{entry.participants.map(item => `${item.display_name} ${money(item.amount_cents)}`).join(' · ')}</p></div>{entry.review_note && <small className="collection-review-note">审核说明：{entry.review_note}</small>}{entry.status === 'pending' && <div className="collection-entry-actions">{isOwner && <><button className="approve" disabled={busy} onClick={() => review(entry.id, 'approved')}><Check size={14}/>通过</button><button className="reject" disabled={busy} onClick={() => review(entry.id, 'rejected')}><X size={14}/>拒绝</button></>}{entry.user_id === user.id && <button disabled={busy} onClick={() => withdraw(entry.id)}><Trash2 size={14}/>撤回</button>}</div>}</article>)}{!selectedPlan.entries.length && <div className="empty-state"><CircleDollarSign size={18}/>还没有成员提交垫付款</div>}</div>

        {selectedPlan.status !== 'collecting' && <div className="manager-panel collection-transfers"><div className="manager-head"><div><h3>最小现金流转账方案</h3><p>付款人先确认已付款，收款人核实到账后再确认收款</p></div><span>{selectedPlan.totals.completed_transfers}/{selectedPlan.totals.transfer_count}</span></div>{selectedPlan.transfers.map(transfer => <div className="collection-transfer" key={transfer.id}><div><strong>{transfer.from_name} <ArrowUpRight size={13}/> {transfer.to_name}</strong><small>{transfer.status === 'pending' ? '等待付款' : transfer.status === 'awaiting_receipt' ? '已付款，等待收款确认' : '双方已确认'}</small></div><b>{money(transfer.amount_cents)}</b>{transfer.from_user_id === user.id && transfer.status === 'pending' && <button className="primary-btn small" disabled={busy} onClick={() => confirmTransfer(transfer.id, 'confirm-payment')}>确认已付款</button>}{transfer.to_user_id === user.id && transfer.status === 'awaiting_receipt' && <button className="primary-btn small" disabled={busy} onClick={() => confirmTransfer(transfer.id, 'confirm-receipt')}>确认已收款</button>}{transfer.status === 'completed' && <span className="collection-done"><CheckCircle2 size={14}/>已完成</span>}</div>)}{!selectedPlan.transfers.length && <div className="empty-state"><CheckCircle2 size={18}/>成员收支已经平衡，无需额外转账</div>}</div>}

        {selectedPlan.member_summaries.length > 0 && <div className="manager-panel collection-balances"><div className="manager-head"><div><h3>结算明细</h3><p>实付减去应承担金额，正数为应收、负数为应付</p></div></div>{selectedPlan.member_summaries.map(item => <div key={item.user_id}><strong>{item.display_name}</strong><span>实付 {money(item.paid_cents)}</span><span>应承担 {money(item.owed_cents)}</span><b className={item.balance_cents >= 0 ? 'positive' : 'negative'}>{item.balance_cents >= 0 ? '+' : '-'}{money(Math.abs(item.balance_cents))}</b></div>)}</div>}
      </div>}
    </div>}
    {showCreate && <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && setShowCreate(false)}><form className="modal" onSubmit={createPlan}><button type="button" className="modal-close" onClick={() => setShowCreate(false)}><X size={18}/></button><span className="modal-kicker">COLLECTION PLAN</span><h2>发起群组收款计划</h2><p>同一群组一次只进行一个计划，结算后可以继续发起下一次。</p><label>计划名称<input name="title" required maxLength="120" placeholder="例如：九月旅行共同支出"/></label><label>计划说明<textarea name="description" rows="3" maxLength="1000" placeholder="说明本次收款范围和截止时间"/></label><button className="primary-btn" disabled={busy}>{busy ? '创建中...' : '确认发起'} <ArrowUpRight size={16}/></button></form></div>}
  </section>
}
...
function TravelRoutesView({ groupId, _detail, canManage, notify }) {
  ...
  const [autoPlanConfig, setAutoPlanConfig] = useState(null)
  ...
  const autoPlan = async event => {
    event.preventDefault(); const data = new FormData(event.currentTarget)
    const config = {travel_date:data.get('travel_date') || null, max_play_minutes:Number(data.get('max_play_minutes') || 600), earliest_start:data.get('earliest_start') || '09:00', latest_end:data.get('latest_end') || '21:00', transport:data.get('transport') || plan.default_transport, prioritize:data.get('prioritize') || 'balanced'}
    await run(async () => { const result = await api(`/travel-plans/${plan.id}/auto-plan`, {method:'POST', body:JSON.stringify(config)}); setAutoPlanConfig(config); setAutoOptions(result.options || []) }, '已生成多套路线方案')
  }
  const applyOption = async optionId => { await run(async () => { await api(`/travel-plans/${plan.id}/apply-plan`, {method:'POST', body:JSON.stringify({option_id:optionId, ...(autoPlanConfig || {})})}); setAutoOptions([]); setAutoPlanConfig(null); await refreshPlan() }, '路线方案已应用') }
...
function LocationPickerMap({ locations, onPick }) {
  ...
  const center = points[0] || [31.2304, 121.4737]
  return <MapContainer {...MAP_OPTIONS} center={center} zoom={12} className="location-map"><MapEffects center={center} zoom={12} points={points} fitBounds={false}/><MapTiles/><MapClickHandler onPick={onPick}/>{locations.map((location, index) => { const point = getMapPoint(location); return point ? <Marker key={`${location.id || index}-${point[0]}-${point[1]}`} position={point}><Popup>{location.name}<br/>{location.address}</Popup></Marker> : null })}</MapContainer>
}

function MapClickHandler({ onPick }) {
  useMapEvents({ click: event => onPick({ name: '地图选点', address: `${event.latlng.lat.toFixed(6)}, ${event.latlng.lng.toFixed(6)}`, lat: event.latlng.lat, lng: event.latlng.lng }) })
  return null
}
...
function ActivityCard({ activity, canManage, canPublish, onVote, onEnd, onPublish, notify }) {
  ...
  const publish = async () => { const selected = activity.slots.filter(slot => publishChoices[slot.id]); if (!selected.length) return notify?.('请至少选择一个候选时间段后再发布'); setPublishing(true); try { await onPublish(activity.id, selected.map(slot => ({title:publishTitles[slot.id] || slot.suggestion || '群组活动', start_at:slot.start_at, end_at:slot.end_at, note:''})), publishNotice, selectedLocation || null); setShowPublish(false) } finally { setPublishing(false) } }
...
function CalendarView({ notify }) {
  ...
  useEffect(() => { load() }, [load])
  const saveAvailability = async event => { event.preventDefault(); if (busy) return; const form = event.currentTarget; const data = new FormData(form); const visibility = data.get('visibility'); setBusy(true); try { await api('/calendar/availability', {method:'POST', body:JSON.stringify({start_at:data.get('start_at'), end_at:data.get('end_at'), visibility, group_id:visibility === 'group' ? Number(data.get('group_id')) : null, note:data.get('note') || ''})}); form.reset(); await load(); notify('空闲时间已保存') } catch (error) { notify(error.message) } finally { setBusy(false) } }
